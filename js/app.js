...
var Enemy = function (startX, startY, speed) {
    ...
    // a helper we've provided to leasily load images
    this.sprite = 'images/EnemyF.png';
    this.x = startX;
    this.y = startY;
    this.speed = speed;
};
...
Enemy.prototype.update = function (dt) {
    ...
    if (player.x > enemyXleftMax && player.x < enemyXRightMax && player.y > enemyYTopMax && player.y < enemyYBottomMax) {
        // my alert
        alert("You've recieved your first L burger.");
        player.resetPlayer();
    }
};
...
var startX = 400;
var startY = 500;
var Player = function (keyInput) {

    this.character = 'images/VegetaZ2.gif-original';
    this.x = startX;
    this.y = startY;
    this.borderCheck = {
        leftWall: false,
...
Player.prototype.handleInput = function (keyInput) {
    ...
    if (keyInput === "left") {
        ...
        if (this.y === 50) {
            // my alert
            alert("Victory always comes in time, good job!");
            this.resetPlayer();
            return null;
        }
        this.y -= moveUpDown;
